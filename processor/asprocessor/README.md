...
The asn processor `asnprocessor` enhances the attributes of a span, log, or metric by appending information about the autonomous system of an IP address. To add autonomous system information, the IP address must be included in the attributes using the [`source.address` or `destination.address` semantic conventions key attribute](https://github.com/open-telemetry/semantic-conventions/blob/v1.26.0/docs/general/attributes.md#source). By default, only the resource attributes will be modified. Please refer to [config.go](./config.go) for the config spec.
